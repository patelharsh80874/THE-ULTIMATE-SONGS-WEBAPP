...
const AlbumDetails = () => {
  ...
  const fetchDetails = async () => {
    setLoading(true);
    try {
      const { data } = await getAlbumDetails(finalid);
    ...
    <div className="w-full min-h-screen bg-slate-900 text-zinc-300 pb-32 overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <div 
          ...
          style={{ 
            backgroundImage: `url(${albumData.image?.[2]?.url || noimg})`,
      ...
      <div className="relative z-10 pt-[12vh] px-8 sm:px-4 max-w-7xl mx-auto">
        ...
        <div className="flex sm:flex-col gap-10 sm:gap-6 items-end sm:items-center mb-16 sm:mb-10">
          ...
          >
            <img
              ...
              src={albumData.image?.[2]?.url || noimg}
              alt={albumData.name}
            />
            <button 
          ...
          <div className="flex-1 flex flex-col sm:items-center sm:text-center pb-2">
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-3 mt-4 sm:mt-3 text-zinc-300 font-medium sm:justify-center">
                <p className="text-xl sm:text-sm text-green-400 font-bold hover:underline cursor-pointer">
                  {albumData.artists?.primary?.[0]?.name}
                </p>
                <div className="w-1.5 h-1.5 bg-zinc-600 rounded-full" />
                <p className="text-xl sm:text-sm">{songs.length} Tracks</p>
                <div className="w-1.5 h-1.5 bg-zinc-600 rounded-full" />
                <p className="text-xl sm:text-sm uppercase">{albumData.language}</p>
              </div>
            </motion.div>
          </div>
        </div>
        ...
        {/* Footer info */}
        <div className="mt-16 py-8 border-t border-white/5 text-center">
          <p className="text-zinc-500 text-xs font-medium italic">
            Released in {albumData.year} · {albumData.copyright || "All Rights Reserved"}
          </p>
        </div>
      </div>
